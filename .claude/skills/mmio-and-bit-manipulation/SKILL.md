...
- iPod targets are classic ARM (ARM7TDMI on PP5022, ARM926EJ-S on S5L8702),
  not Cortex-M: no CMSIS, no BSRR-style set/clear registers unless the SoC
  datasheet says so. Barriers on ARMv4/v5 are `asm volatile("" ::: "memory")`
  compiler barriers plus SoC-specific drain/cache ops.
- Register definitions live in `firmware/export/pp5020.h` / `s5l8702.h` and
  target headers under `firmware/target/arm/`; follow the existing macro
  style there instead of inventing new REG macros.
- Rockbox code style: 4-space indent, `/* comments */`, no typedefs.
